...
export type StructuredDiffVisibleItem =
  | StructuredDiffVisibleRow
  | StructuredDiffGap;
...
function computeInlineHighlights(
  oldText: string,
  newText: string,
): { oldHighlights: InlineRange[]; newHighlights: InlineRange[] } {
  if (oldText.length === 0) {
  ...
  if (oldText.length + newText.length > INLINE_HIGHLIGHT_CHAR_LIMIT) {
    return {
      oldHighlights: fullHighlight(oldText),
      newHighlights: fullHighlight(newText),
    };
  }
...
function buildStructuredDiffFromRows(
  ...
  const additions = rows.reduce(
    (count, row) =>
      count + (row.kind === "insert" || row.kind === "replace" ? 1 : 0),
    0,
  );
  const deletions = rows.reduce(
    (count, row) =>
      count + (row.kind === "delete" || row.kind === "replace" ? 1 : 0),
    0,
  );
  ...
  if (changeBlocks.length === 0) {
    return {
      rows,
      visibleItems: rows.map((row, fullRowIndex) => ({
        type: "row",
        fullRowIndex,
        row,
      })),
  ...
  const hunks: StructuredDiffHunk[] = seeds.map((seed, index) => {
    const oldRange = getLineRange(
      rows,
      seed.changeStartRow,
      seed.changeEndRow,
      "old",
    );
    const newRange = getLineRange(
      rows,
      seed.changeStartRow,
      seed.changeEndRow,
      "new",
    ...
    for (
      let rowIndex = seed.changeStartRow;
      rowIndex <= seed.changeEndRow;
      rowIndex++
    ) {
  ...
  for (const hunk of hunks) {
    if (hunk.displayStartRow > cursor) {
      ...
      visibleItems.push({
        ...
        label: createGapLabel(
          cursor === 0 ? "start" : "middle",
          hiddenRowCount,
        ),
      });
    }
    ...
    for (
      let rowIndex = hunk.displayStartRow;
      rowIndex <= hunk.displayEndRow;
      rowIndex++
    ) {
      visibleItems.push({
        type: "row",
...
export function buildStructuredDiff(
  ...
  return buildStructuredDiffFromRows(
    aligned.rows,
    aligned.totalOldLines,
    aligned.totalNewLines,
    contextLines,
  );
}

export function adjustStructuredDiffContext(
  diff: StructuredDiff,
  contextLines: number,
): StructuredDiff {
  return buildStructuredDiffFromRows(
...
export function renderUnifiedDiff(diff: StructuredDiff): string {
  ...
  for (const item of diff.visibleItems) {
    if (item.type === "gap") {
      output.push(
        formatUnifiedLine(" ", undefined, item.label, lineNumberWidth),
      );
      continue;
    }
    ...
    if (row.kind === "equal") {
      output.push(
        formatUnifiedLine(" ", row.oldLineNumber, row.oldText, lineNumberWidth),
      );
      continue;
    }
    if (row.kind === "delete") {
    ...
    if (row.kind === "insert") {
      output.push(
        formatUnifiedLine("+", row.newLineNumber, row.newText, lineNumberWidth),
      );
      continue;
    }
